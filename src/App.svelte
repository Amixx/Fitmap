<script lang="ts">
  ...
  const bodyTiers = [['hats', 'scarves'], ['outerwear', 'tops'], ['bottoms'], ['socks', 'shoes'], ['gloves', 'bags']];
  ...
  function bodyRows(itemIds: string[]) {
    const pieces = itemIds.map((id) => items.find((item) => item.id === id)).filter((item): item is Item => Boolean(item));
    const rows = bodyTiers.map((tier) => rarelyUsedLast(pieces.filter((item) => tier.includes(item.category)).sort((first, second) => tier.indexOf(first.category) - tier.indexOf(second.category))));
    const placed = new Set(rows.flat().map((item) => item.id));
    return [...rows, rarelyUsedLast(pieces.filter((item) => !placed.has(item.id)))].filter((row) => row.length > 0);
  }
  function bodyOrder(itemIds: string[]) {
    return bodyRows(itemIds).flat();
  }
